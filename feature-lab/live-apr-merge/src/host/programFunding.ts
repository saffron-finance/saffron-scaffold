...
export const programWithdrawalStorageKey=(account:string,id:string)=>'saffron.campaign-withdrawal.v1:'+account.toLowerCase()+':'+id
...
export function programWithdrawalQuote(value:any,now=Date.now()){
...
export function programFundingTerms(row:Deployment,now=Date.now()){
  ...
  if(row.workerState!=='created'||row.cancelRequested||row.refund||s.isStarted)throw new Error('This vault is no longer accepting incentive program funding.')
...
export function programFundingAction(terms:ReturnType<typeof programFundingTerms>,allowance:bigint):{stage:string;label:string;to:Address;data:Hex;value:bigint}{
  ...
  return {stage:'fund',label:'Fund incentive program',to:vault,value:0n,
