...
export function Sidebar({ home, collapsed, onToggle }: { home: string; collapsed: boolean; onToggle: () => void }) {
  ...
  return <Rail data-saffron-sidebar data-collapsed={collapsed} aria-label='Saffron sidebar'>
    ...
    <Navigation id={navigationId} aria-label='Main navigation' aria-hidden={collapsed || undefined}>
      {sidebarDestinations(home).map(item => <Fragment key={item.href}>{item.icon==='admin'&&<GroupLabel>Operator workspace</GroupLabel>}<NavItem to={item.href}
...
const GroupLabel=styled.span`display:block;margin:18px 12px 2px;padding-top:18px;border-top:1px solid #303037;color:#a9a2af;font-size:10px;text-transform:uppercase;letter-spacing:.08em;`
