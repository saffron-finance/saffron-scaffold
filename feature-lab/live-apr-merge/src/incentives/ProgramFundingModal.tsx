...
export function ProgramFundingModal({account,row,onClose}:{account:Address;row:Deployment;onClose:()=>void}){
  ...
  return <Modal isOpen contentLabel='Fund incentive program' onRequestClose={close} shouldCloseOnOverlayClick={!flow.closeBlocked}
    contentStyle={{padding:'20px 24px 26px',maxWidth:560,width:'calc(100vw - 32px)',boxSizing:'border-box'}}>
    <Stack data-program-funding={row.id}>
      <Row><ModalTitle style={{margin:0}}>Fund incentive program</ModalTitle><QuietButton aria-label='Close incentive program funding' disabled={flow.closeBlocked} onClick={close}>×</QuietButton></Row>
      ...
      {flow.completed?<b role='status'>Incentive program funding confirmed.</b>:flow.quote?<>
        ...
        <FinePrint>Approve the displayed token amount, then confirm “Fund incentive program” separately in your wallet.</FinePrint>
      ...
      {flow.pending?<Stack>
        ...
        <PrimaryAction disabled={flow.busy||!flow.quote||Boolean(flow.quote.blocked)} onClick={()=>void flow.advance()}>{flow.busy?(flow.closeBlocked?'Confirming wallet action…':'Preparing wallet action…'):flow.quote?.action.label??'Fund incentive program'}</PrimaryAction>
