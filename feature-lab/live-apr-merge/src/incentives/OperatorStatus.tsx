...
export function OperatorStatus({account,onConnect,onNavigate}:{account:Address|null;onConnect:()=>void;onNavigate:(path:string)=>void}){
  ...
  return <Stack>
    <OpsCard><OpsRow><div><h2>Server controls live in a separate console</h2><p>This page reads the same backend checks. Green means verified; blocked items include repair instructions.</p></div><a href={operatorConsoleHref} target="_blank" rel="noopener" style={{color:"#d286ff",padding:"12px",border:"1px solid #755285",borderRadius:6}}>Open Server operator ↗</a></OpsRow></OpsCard>
    ...
    {report&&<>
      ...
      <OpsCard><h2>Wallet roles</h2><p>The admin wallet signs management actions. The unrestricted factory deployer wallet pays gas for vault creation. The fee recipient receives user request fees. These roles are not interchangeable.</p>
        <p>Configured unrestricted factory deployer: <code>{report.signer??'Not configured'}</code><br/>Fee recipient: <code>{report.feeRecipient??'Not configured'}</code></p>
