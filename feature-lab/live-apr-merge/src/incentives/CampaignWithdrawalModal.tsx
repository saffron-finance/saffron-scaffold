...
export function CampaignWithdrawalModal({account,row,onClose}:{account:Address;row:Deployment;onClose:()=>void}){
  ...
  return <Modal isOpen contentLabel='Withdraw from vault' onRequestClose={close} shouldCloseOnOverlayClick={!flow.closeBlocked}
    ...
    <Stack data-campaign-withdrawal={row.id}>
      <Row><ModalTitle style={{margin:0}}>Withdraw from vault</ModalTitle><QuietButton aria-label='Close campaign withdrawal' disabled={flow.closeBlocked} onClick={close}>×</QuietButton></Row>
