...
import type {ProgramCatalog} from './program-admin'
...
function show(create=false){return render(<ThemeProvider theme={darkTheme}>{create?<CreateIncentiveProgramPage account={account} onConnect={()=>{}} onBack={navigate}/>:<ProgramAdmin autoLoad account={account} onConnect={()=>{}} onCreate={navigate}/>}</ThemeProvider>)}
beforeEach(()=>{history.replaceState(null,'','/incentive-programs');navigate.mockReset();api.authed.mockReset();catalog={pairs:[pair],programs:[program],budgets:[budget]};api.authed.mockImplementation(async(_account,path)=>{if(path==='/admin/catalog')return structuredClone(catalog);return {ok:true}})})
afterEach(cleanup)
...
it('keeps each fee and target in its program row, and creation out of the manager',async()=>{
  ...
  show();await screen.findByRole('list',{name:'Incentive programs'})
  for(const p of catalog.programs){
    fireEvent.click(screen.getByRole('button',{name:'Open incentive program '+programReference(p.id)}))
    const row=screen.getByRole('region',{name:'Incentive program details'})
    expect(within(row).getByText(programReference(p.id))).toBeVisible()
    expect(within(row).getByLabelText('Request fee ETH for '+programReference(p.id))).toBeVisible()
    expect(within(row).getByLabelText('Planning budget for '+programReference(p.id))).toBeVisible()
    expect(within(row).getByText('Pause / resume affects all programs sharing this budget.')).toBeVisible()
    fireEvent.click(screen.getByRole('button',{name:'All incentive programs'}))
  }
  ...
  fireEvent.click(screen.getByRole('button',{name:'New incentive program'}));expect(navigate).toHaveBeenCalledOnce()
})

it('saves an exact 18-decimal request fee without changing other program settings',async()=>{
  show();fireEvent.click(await screen.findByRole('button',{name:'Open incentive program '+programReference(program.id)}));const input=await screen.findByLabelText('Request fee ETH for '+programReference(program.id))
...
it('retains a failed fee edit for correction without announcing success',async()=>{
  ...
  expect(screen.queryByText('Incentive program configuration saved.')).toBeNull()
})
...
it('creates only on explicit submission, preserves idempotency on failure, and excludes the calculated value',async()=>{
  ...
  expect(screen.queryByRole('table',{name:'Incentive program configuration'})).toBeNull()
  expect(screen.getByLabelText('Incentive program APR percent')).toHaveAttribute('readonly')
  expect(screen.getByRole('button',{name:'Create incentive program'})).toBeDisabled()
  ...
  fireEvent.click(screen.getByRole('button',{name:'Create incentive program'}));await screen.findByText('Reply unavailable; retry.')
...
it('clearly distinguishes calculated capacity and rejects invalid economics or fees',async()=>{
  show(true);await screen.findByRole('form',{name:'Create incentive program'})
  fireEvent.change(screen.getByLabelText('Calculate incentive program field'),{target:{value:'capacity'}})
  expect(screen.getByLabelText('Incentive program capacity USD')).toHaveAttribute('readonly')
  expect(screen.getByLabelText('Incentive program APR percent')).not.toHaveAttribute('readonly')
  fireEvent.change(screen.getByLabelText('Incentive program request fee ETH'),{target:{value:'0.0000000000000000001'}})
  expect(screen.getByRole('button',{name:'Create incentive program'})).toBeDisabled()
  fireEvent.change(screen.getByLabelText('Incentive program request fee ETH'),{target:{value:'0.001'}})
  fireEvent.change(screen.getByLabelText('Incentive program duration'),{target:{value:'0'}})
  expect(screen.getByRole('button',{name:'Create incentive program'})).toBeDisabled()
  fireEvent.change(screen.getByLabelText('Incentive program duration'),{target:{value:'7'}})
  fireEvent.click(screen.getByRole('button',{name:'Create incentive program'}))
  await waitFor(()=>expect(navigate).toHaveBeenCalledOnce())
...
it('shows empty-pair and failed-catalog states without a creation form',async()=>{
  ...
  expect(screen.queryByRole('form',{name:'Create incentive program'})).toBeNull();view.unmount()
