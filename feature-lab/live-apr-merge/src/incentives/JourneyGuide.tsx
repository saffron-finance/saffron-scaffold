...
/** Real workflow guidance, not example incentive program state or a simulated journey. */
export function StartupSteps({onNavigate}:{onNavigate:(path:string)=>void}){
  return <OpsCard aria-label='How to turn on paid requests'><h2>How to turn on paid requests</h2>
    <ol>
      <li><b>You — prepare the incentive program.</b> Connect an allowlisted admin wallet, sign in, and configure the pool, duration, economics, and fixed ETH request fee. Arrange an external premium funder and refund payer.</li>
      ...
      <li><b>You + server operator — prepare the unrestricted factory deployer.</b> Use an unrestricted factory deployer wallet. Create the key off this server. Follow the separate operator console’s secure SSH import instructions. Its import script asks once with hidden Bash input and stores the key in the password manager. Never enter the key in chat or a browser. Supply Robinhood ETH for gas. The deployer must use the same public signer and protocol settings as the API.</li>
      <li><b>Server operator — run and supervise creation.</b> Start the unrestricted factory deployer service and verify its heartbeat. Check its logs and journal for failed or pending work. This page does not start a signer.</li>
    ...
    <OpsButtons><QuietButton onClick={()=>onNavigate('/incentive-programs')}>Incentive programs</QuietButton><QuietButton onClick={()=>onNavigate('/admin')}>Administration</QuietButton><QuietButton onClick={()=>onNavigate('/status')}>Status</QuietButton></OpsButtons>
    <details><summary>Server operator commands and restart rules</summary><p>After protected configuration is provisioned, the repository provides two separate runners. These commands are instructions, not browser actions.</p>
      ...
      <p><code>npm run worker -- &lt;protected-creator-config&gt;</code><br/>This enables automatic transaction signing for accepted requests. The key belongs only in the unrestricted factory deployer process.</p>
...
export function JourneyGuide({onNavigate}:{onNavigate:(path:string)=>void}){
  const stages=[['1 · Request','The user connects a wallet, selects an incentive program and LP size, and pays the exact quoted ETH fee. This fee is fixed per incentive program, not linked to ETH/USD.'],['2 · Create','The watcher verifies the payment. The unrestricted factory deployer deploys the adapter and vault, then initializes the saved terms.'],['3 · Fund premium','The external funder supplies the full required premium. Creation alone does not make a vault ready for an LP deposit.'],['4 · Deposit and claim','The user wallet approves and deposits the required LP assets, then claims the incentive. On-chain ownership controls these actions.'],['5 · Mature and withdraw','The position remains locked for the chosen term. At maturity, the user wallet withdraws through Portfolio.'],['If progress stops','Use Administration for saved requests, payment exceptions and refund review. Status explains service blockers. Preserve the original payment; do not pay again because a callback failed.']]
