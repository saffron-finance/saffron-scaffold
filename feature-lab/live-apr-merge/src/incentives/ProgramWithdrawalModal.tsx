...
export function ProgramWithdrawalModal({account,row,onClose}:{account:Address;row:Deployment;onClose:()=>void}){
  ...
  return <Modal isOpen contentLabel='Withdraw from vault' onRequestClose={close} shouldCloseOnOverlayClick={!flow.closeBlocked}
    contentStyle={{padding:'20px 24px 26px',maxWidth:560,width:'calc(100vw - 32px)',boxSizing:'border-box'}}>
    <Stack data-program-withdrawal={row.id}>
      <Row><ModalTitle style={{margin:0}}>Withdraw from vault</ModalTitle><QuietButton aria-label='Close incentive program withdrawal' disabled={flow.closeBlocked} onClick={close}>×</QuietButton></Row>
      ...
      {flow.completed?<b role='status'>Incentive program withdrawal confirmed.</b>:flow.quote?<>
      ...
      {flow.pending?<Stack>
        ...
        <PrimaryAction disabled={flow.busy||(!flow.pending.hash&&!/^0x[0-9a-fA-F]{64}$/.test(hash))} onClick={()=>void flow.recover(hash?hash as Hex:undefined)}>{flow.busy?(flow.closeBlocked?'Confirming wallet action…':'Preparing wallet action…'):'Check withdrawal transaction'}</PrimaryAction>
      </Stack>:!flow.completed?<>
        <QuietButton disabled={flow.busy} onClick={()=>void flow.refresh()}>Refresh withdrawal</QuietButton>
        <PrimaryAction disabled={flow.busy||!flow.quote||Boolean(flow.quote.blocked)} onClick={()=>void flow.advance()}>{flow.busy?(flow.closeBlocked?'Confirming wallet action…':'Preparing wallet action…'):flow.quote?.action.label??'Withdraw from vault'}</PrimaryAction>
