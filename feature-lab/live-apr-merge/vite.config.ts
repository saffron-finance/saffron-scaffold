...
export default defineConfig(({ mode }) => {
  ...
  plugins: [react(), svgr(), {name:'separate-incentive-mode',enforce:'pre',resolveId(source,importer){
    ...
    resolveId(source) {
      if (mode !== 'lab' && source === '../dev/RowTweaks') return '\0no-tweaks'
    },
    load(id) { if (id === '\0no-tweaks') return 'export default function NoTweaks(){return null}' },
  }],
  define: { 'import.meta.env.VITE_DEV_TWEAKS': JSON.stringify(mode === 'lab' ? 'true' : 'false') },
  resolve: {
    alias: {
