...
const {exercisePairPicker}=await import(pathToFileURL(path.resolve(backend,'tests/browser/admin-pair-journey.mjs')).href)
...
try {
  ...
  await menuLink('Incentive programs')
  await page.getByRole('button',{name:'New incentive program',exact:true}).click()
  await expect(page.getByLabel('Incentive program request fee ETH')).toHaveValue('')
  await page.getByLabel('Incentive program request fee ETH').fill('0.0000000000000000001')
  await expect(page.getByRole('button',{name:'Create incentive program',exact:true})).toBeDisabled()
  await expect(page.getByRole('form',{name:'Create incentive program',exact:true}).getByRole('alert')).toContainText('at most 18 decimal places')
  await page.getByLabel('Incentive program request fee ETH').fill('0.000000000000000007')
  ...
  const created=(await fixture.database.catalog(true)).programs.find(p=>p.id.startsWith('campaign-'))
  assert(created,'Incentive program ID is assigned without an input')
  const reference=created.id.replace(/^campaign-/,'program-')
  await page.getByRole('button',{name:'Open incentive program '+reference,exact:true}).click()
  await expect(page.getByLabel('Request fee ETH for '+reference)).toHaveValue('0.000000000000000007')
  await page.getByLabel('Request fee ETH for '+reference).fill('0.123456789012345678')
  await page.getByRole('form',{name:'Request fee for '+reference,exact:true}).getByRole('button',{name:'Save request fee',exact:true}).click()
  await expect.poll(async()=>(await fixture.database.catalog(true)).programs.find(p=>p.id===created.id).requestFeeWei).toBe('123456789012345678')
  assert(fixture.state.signs>0,'Operator changes require a real wallet signature')
  await page.reload();await expect(page.getByLabel('Request fee ETH for '+reference)).toHaveValue('0.123456789012345678')
  await page.getByRole('button',{name:'All incentive programs',exact:true}).click()
  await exercisePairPicker(page,fixture,expect)
  report.checks.push('Auto-assigned campaign IDs and pool headings; searchable token pickers, duplicate-token protection, chain metadata, swap, fee tiers, keyboard and mobile save')
  ...
  await page.waitForURL('**/incentive-programs?keep=1#example')
