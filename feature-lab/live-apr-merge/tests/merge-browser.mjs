...
try {
  ...
  assert(created,'Campaign ID is assigned without an input')
  await expect(page.getByLabel('Request fee ETH for '+created.id)).toHaveValue('0.000000000000000007')
  await page.getByLabel('Request fee ETH for '+created.id).fill('0.123456789012345678')
  await page.getByRole('form',{name:'Request fee for '+created.id,exact:true}).getByRole('button',{name:'Save request fee',exact:true}).click()
  ...
  await page.reload();await expect(page.getByLabel('Request fee ETH for '+created.id)).toHaveValue('0.123456789012345678')
