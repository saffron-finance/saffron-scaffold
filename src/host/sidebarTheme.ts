...
// One selected surface for navigation and the two modal primary actions.
// Geometry and typography remain with each control; only paint is shared.
export const sidebarSelectedSurface = `
  color:var(--sidebar-selected-text);border-color:var(--sidebar-button-border);
  background:var(--sidebar-button-background);background-size:var(--sidebar-button-size);
  animation:var(--sidebar-button-motion);
...
export function sidebarVariables(value: SidebarAppearance): string {
  ...
  const approvedPaint = value.style === 'afterglow' ? `
    ...
    --sidebar-hover-text:var(--sidebar-selected-text);
  ` : ''
  return `
    ...
    /* Keep inactive-link hover neutral, independently of the selected palette. */
    --sidebar-hover-background: #1d1d1d; --sidebar-hover-text:#f2f0ea;
