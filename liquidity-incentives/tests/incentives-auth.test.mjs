...
it('wallet sessions bind origin, nonce, wallet and CSRF; operators are a separate permission',async()=>{
  ...
  assert.throws(()=>auth.challenge(request,user.address),e=>e.status===403)
  const session=auth.grantPayment(request,{setHeader:(key,value)=>{headers[key]=value}},user.address)
  ...
  const proof=auth.challenge(request,operator.address),signature=await operator.signMessage({message:walletSessionMessage(proof)})
  assert.equal((await auth.login(request,{setHeader(){}},{wallet:operator.address,nonce:proof.nonce,signature})).operator,true)
  await assert.rejects(auth.login(request,{setHeader(){}},{wallet:operator.address,nonce:proof.nonce,signature}),e=>e.status===401)
