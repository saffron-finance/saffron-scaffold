...
contract UniV3LimitedRangeAdapter is AdapterBase, IUniV3Adapter {
  ...
  /// @notice Upper bound tolerance for liquidity in basis points (e.g., 10100 = +1%)
  uint256 public upperDepositTolerance;
  ...
  function initialize(
    ...
    invDepositTolerance = VaultConstants.MAX_BPS - _depositTolerance;
    // Set upper bound tolerance: use the same value as lower bound for symmetric protection
    // Alternative: could add a fixed buffer, e.g., _depositTolerance + 50 (additional 0.5%)
    upperDepositTolerance = _depositTolerance;
    id = _id;
    ...
    // Validate pool authenticity against Uniswap V3 Factory
    ...
    require(expectedPool == _pool, "Invalid pool");
    ...
    require(TickMath.MIN_TICK <= params.minTick && params.minTick < params.maxTick && params.maxTick <= TickMath.MAX_TICK, "IT");
  ...
  function _mintToken(
    ...
    // Enforce lower bound liquidity check (existing)
    require(uint256(l) * invDepositTolerance / VaultConstants.MAX_BPS < _liquidity, "L");

    // Enforce upper bound liquidity check to prevent excessive liquidity
    require(_liquidity <= uint256(l) * (VaultConstants.MAX_BPS + upperDepositTolerance) / VaultConstants.MAX_BPS, "Excessive liquidity");
  ...
  function setVault(address _vaultAddress) public override(AdapterBase, IAdapter) {
  ...
  function settleEarnings() external override onlyVault returns (uint256, uint256) {
    ...
    (earnings0, earnings1) = _collect(tokenId);
  ...
  function _removeLiquidity(bytes calldata removeLiquidityData) internal returns (uint256 amount0, uint256 amount1) {
    ...
    (uint256 amount0, uint256 amount1) = _collect(_tokenId);
    positionManager.burn(_tokenId);
    emit LiquidityRemoved(vaultAddress, address(pool), _tokenId, _liquidity, amount0, amount1);
    tokenId = 0;
    liquidity = 0;
    return (amount0, amount1);
  }

  function _collect(uint256 _tokenId) internal returns (uint256 _earnings0, uint256 _earnings1) {
  ...
  function depositTolerance() public view returns (uint256) {
    return VaultConstants.MAX_BPS - invDepositTolerance;
  }
}
