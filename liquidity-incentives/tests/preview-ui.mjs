...
try{
  ...
  await expect(page.locator('[data-incentive-offer]').getByText(/capacity/i)).toHaveCount(0)
  ...
  await writeFile(output+'/verification.json',JSON.stringify({ok:true,root,staticOnly:true,walletCalls:0,appearance,errors,requests,checks:['approved font families loaded locally','original sidebar/card/header geometry','Vaults homepage and all menu routes','desktop/mobile/compact navigation','exact shared campaign calculator','half-funded sample','local payment review/creation','private planning accounting updates after simulated payment','campaign creation persists locally','deep-link reload','desktop/390/320 layout','no API/RPC/price/wallet calls']},null,2))
  console.log('Preview browser checks passed: calculator, payment review, private planning accounting, saved campaign, deep-link reload, mobile and no live calls.')
