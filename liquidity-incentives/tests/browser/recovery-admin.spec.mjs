...
test('campaign form derives APR, budget or capacity and stores a paused campaign without moving funds',async({page})=>{
  ...
  try{
    ...
    await page.getByLabel('Incentive program request fee ETH').fill('0.001')
    await expect(page.getByLabel('Incentive program APR percent')).toHaveValue('121.666667')
    await expect(page.getByLabel('Incentive program APR percent')).toHaveAttribute('readonly','')
    await page.getByLabel('Calculate incentive program field').selectOption('capacity')
    await page.getByLabel('Incentive program APR percent').fill('100')
    await expect(page.getByLabel('Incentive program capacity USD')).toHaveValue('1216666.66')
    await page.getByLabel('Calculate incentive program field').selectOption('budget')
    await expect(page.getByLabel('Incentive program budget USD')).toHaveValue('8219.18')
    await page.getByLabel('Calculate incentive program field').selectOption('apr')
    ...
    await expect(page.getByLabel('Incentive program ID',{exact:true})).toHaveCount(0)
    await expect(page.getByLabel('Incentive program name',{exact:true})).toHaveCount(0)
