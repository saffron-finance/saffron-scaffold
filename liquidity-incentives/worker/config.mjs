...
export async function readOperatorConfig(file,{oneRequest=false,readOnly=false}={}){
  ...
  for(const name of ['vaultTypeId','adapterTypeId','maxGasPerTx','maxGasPriceWei'])if(!/^[1-9][0-9]*$/.test(String(config[name]??'')))throw new Error('Positive execution limits are required.')
  ...
  // Legacy one-request files use this old field name. It is not a daily ledger.
  config.maxOneShotGasWei??=config.maxDailyGasWei
  if(oneRequest&&!/^[1-9][0-9]*$/.test(String(config.maxOneShotGasWei??'')))throw new Error('A positive one-shot signing ceiling is required.')
