...
export function AppShell({ account, onConnect, children, overlays }: {
  ...
  return <ThemeProvider theme={light ? lightTheme : darkTheme}>
    ...
    <Modal isOpen={menu} onRequestClose={() => setMenu(false)}>
      ...
      <WalletList>
        <NavLink href={mount}>Vaults</NavLink>
        <NavLink href={mount + 'portfolio/vaults/'}>My requests</NavLink>
        <NavLink href={mount + 'incentive programs/'}>Incentive programs</NavLink>
...
const Footer = styled.footer`padding:40px 20px 24px;text-align:center;font-size:12px;color:${p => p.theme.colors.text.tertiary};a{color:${p => p.theme.colors.text.tertiary}}`
