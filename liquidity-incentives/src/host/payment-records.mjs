...
export function readPayments(storage,wallet){
  ...
  if(raw===null)return {revision:0,activeId:null,records:{}}
  let ledger
  ...
  if(!Number.isSafeInteger(ledger?.revision)||ledger.revision<0||!ledger.records||typeof ledger.records!=='object'||Array.isArray(ledger.records)
    ...
    ||(ledger.activeId!==null&&!Object.hasOwn(ledger.records,ledger.activeId)))throw new Error('Saved payment recovery data is invalid. Do not pay again.')
  return ledger
}
...
export function savePayment(storage,wallet,expected,payment,{active=true}={}){
  ...
  const next={revision:latest.revision+1,activeId:active?id:null,records:{...latest.records,[id]:{...payment,updatedAt:Date.now()}}}
