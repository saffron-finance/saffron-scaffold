...
function WalletPage({account,onConnect,selected,setSelected}:{account:Address|null;onConnect:()=>void;selected:Offer|null;setSelected:(offer:Offer|null)=>void}){
  ...
  const currentRoute=()=>{const path=location.pathname.slice(base.length).replace(/\/+$/,'')||'/';const next=path==='/campaigns'?'/incentive-programs':path==='/campaigns/new'?'/incentive-programs/new':path;if(next!==path)history.replaceState(history.state,'',base+next+location.search+location.hash);return next}
  ...
  useEffect(()=>{const update=()=>setRoute(currentRoute());window.addEventListener('popstate',update);return()=>window.removeEventListener('popstate',update)},[base])
  ...
  return <Page>
    {route==='/status'?<OperatorStatus account={account} onConnect={onConnect} onNavigate={navigate}/>:route==='/journey'?<JourneyGuide onNavigate={navigate}/>:route==='/incentive-programs/new'?<><TitleRow><StepTitle>Create incentive program</StepTitle><QuietButton onClick={()=>navigate('/incentive-programs')}>Back to incentive programs</QuietButton></TitleRow><CreateIncentiveProgramPage account={account} onConnect={onConnect} onBack={()=>navigate('/incentive-programs')}/></>:route==='/incentive-programs'?<><TitleRow><StepTitle>Incentive programs</StepTitle><QuietButton onClick={()=>navigate('/')}>Vaults</QuietButton></TitleRow><ProgramAdmin autoLoad account={account} onConnect={onConnect} onCreate={()=>navigate('/incentive-programs/new')}/></>:route==='/admin'?<IncentivesAdmin account={account} onConnect={onConnect} onNavigate={navigate} onBack={()=>navigate('/')}/>:route==='/portfolio/vaults'?<MyVaults account={account} positions={positions} onConnect={onConnect} onBack={()=>navigate('/')} onOpen={(id,position=false)=>{setVaultId(id);setOpenPosition(position)}} onAdmin={()=>navigate('/admin')} payments={flow.records.filter(p=>p.sent&&!p.deploymentId)} onResumePayment={async(id)=>{await flow.resumePayment(id);setSelected(null);setVaultId(null);setResume(true)}}/>:<>
      ...
      <Introduction aria-label='About liquidity incentives'><StepSubtitle>Choose a liquidity incentive and create a vault sized to your deposit. Each incentive program has a fixed duration and target APR. Review your position and premium before paying the incentive program’s fixed ETH request fee.</StepSubtitle><StepSubtitle>We fund the premium after your vault is created. Once it is ready, deposit your LP assets and claim your incentive. Your position stays locked for the chosen duration; follow its progress and withdraw at maturity from My requests.</StepSubtitle></Introduction>
