...
import { ErrorText,FinePrint,Action as PrimaryAction,Row,Stack } from './styles'
...
export function IncentiveProgramEditor({account,pairs,onBack,onSaved}:{account:Address;pairs:Pair[];onBack:()=>void;onSaved:()=>void}){
  ...
  try{terms=campaignTerms(economics)}catch(e){validation=programText((e as Error).message)}
  ...
  return <Form onSubmit={submit} aria-label='Create incentive program'><FormScope disabled={busy}>
    <Panel aria-labelledby='campaign-pool-title'><SectionIntro><Eyebrow>01 · Pool &amp; term</Eyebrow><h2 id='campaign-pool-title'>Choose the incentive program market</h2></SectionIntro><Fields>
      ...
      <Field>Request fee · ETH *<input aria-label='Incentive program request fee ETH' aria-describedby='campaign-fee-help' required inputMode='decimal' placeholder='e.g. 0.001' value={requestFee} onChange={e=>setRequestFee(e.target.value)}/><FinePrint id='campaign-fee-help'>Fixed ETH per new request, plus wallet network gas. Not a USD price.</FinePrint></Field>
    </Fields>{feeError&&<ErrorText role='alert'>{feeError}</ErrorText>}</Panel>
    <Panel aria-labelledby='campaign-economics-title'><SectionIntro><Eyebrow>02 · Incentive program economics</Eyebrow><h2 id='campaign-economics-title'>Set two values; calculate the third</h2><FinePrint>Planning targets do not cap request count or size. APR uses a 365-day year, without compounding.</FinePrint></SectionIntro>
