...
export const sidebarPresets = [
  { id: 'reference', label: 'Reference (current)', colors: ['#f5d27b', '#e47e01', '#b8842a'], ink: '#f5d27b', glow: 10, strength: 20 },
  ...
  { id: 'afterglow', label: '05 · Violet afterglow', colors: ['#d793ff', '#9a28b8', '#ff6c16'], ink: '#fff0ff', glow: 28, strength: 70 },
...
export function applySidebarPreset(value: SidebarAppearance, id: string): SidebarAppearance {
  ...
  return { ...value, style: preset.id, gradientStart: preset.colors[0], gradientMiddle: preset.colors[1],
    ...
    angle: preset.id === 'reference' ? sidebarDefaults.angle : 110 }
}
...
export function sidebarPresetCss(value: SidebarAppearance): string {
  if (value.style === 'reference') return ''
  ...
  const paints: Record<string, string> = {
    ...
    afterglow: `radial-gradient(110% 180% at 0% 100%,${a},${b} 38%,transparent 75%),linear-gradient(${angle},${base},${c})`,
  ...
  if (!paint) return ''
  return `
    --sidebar-button-background:${paint}; --sidebar-hover-background:var(--sidebar-button-background);
