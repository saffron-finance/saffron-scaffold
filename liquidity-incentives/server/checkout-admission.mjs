...
/** Browser admission limits unpaid checkouts; it proves no wallet ownership. */
  ...
  return {
    ...
    async issue(req,res){
      ...
      await db.transaction(async client=>{
        ...
        const counts=(await client.query(`SELECT count(*)::int total,count(*) FILTER(WHERE peer_hash=$1)::int peer
          FROM saffron_incentives.checkout_clients WHERE created_at>$2`,[peer,new Date(now()-60_000)])).rows[0]
        if(counts.total>=120||counts.peer>=10)throw fault(429,'Checkout admission is busy. Retry shortly.')
